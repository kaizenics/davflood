 ...
 * THE LOADING SEAM.
 *
 * Today the hazard polygons are bundled JSON — Metro inlines them, so they are
 * present before the first frame and the map works with no network at all.
 * That is the whole offline story, and for one city at a few KB per scenario
 * it is comfortably the right call.
 *
 * If the real UP NOAH export turns out to be large enough to hurt startup on a
 * low-end device, THIS FUNCTION is the only thing that changes — swap it for a
 * lazy `expo-file-system` read or a hosted tile source. No component knows
...
/** Feature count, for the "what am I looking at" line on the Learn tab. */
 ...
 * Flip this to `false` in the same commit that lands the real NOAH export.
 */
