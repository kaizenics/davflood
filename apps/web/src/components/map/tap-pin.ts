...
/**
 * A house for a 24×24 box, drawn for that box.
 *
 * The pin's own glyph is a different shape in a 24×32 viewBox — reusing it
 * here and nudging it with a transform put an 11px drawing in the top third
 * of a 13px icon, which read as a smudge rather than a house.
 */
const HOUSE_PATH = "M12 3 2.8 11.2V21h6.4v-5.6h5.6V21h6.4V11.2L12 3Z";
...
export function createTapPin(opts: {
  ...
  button.append(iconSvg(HOUSE_PATH, "nf-pin-save-icon"));
...
function iconSvg(path: string, className: string): SVGSVGElement {
  const svg = document.createElementNS(NS, "svg");
